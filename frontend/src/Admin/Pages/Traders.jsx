...
const Traders = () => {
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(
        `http://localhost:4000/api/traders/${traderToDelete}`,
        {
          headers: { "auth-token": localStorage.getItem("token") },
        }
      );
  ...
  const handleSaveChanges = async (event) => {
    ...
    try {
      ...
      await axios.put(
        `http://localhost:4000/api/traders/${selectedTrader._id}`,
        updatedTrader,
        {
          headers: { "auth-token": localStorage.getItem("token") },
        }
      ...
      setTraders(
        traders.map((trader) =>
          trader._id === selectedTrader._id ? updatedTrader : trader
        )
      );
              ...
              <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg">
                ...
                {selectedTrader && (
                  <form onSubmit={handleSaveChanges}>
                    ...
                    <button
                      type="submit"
                      className="bg-[#ffb400] text-[#01071C] px-4 py-2 rounded-md"
                    >
                      Save Changes
                    </button>
                  </form>
